...
.. glossary::
    ...
    cluster
        A set of peer-to-peer quasardb nodes configured to work as a single distributed :term:`repository`.
    ...
    server
        A running instance of a quasardb daemon. On most setups only one server runs on any given :term:`node`.

    stable
        A :term:`cluster` is stable if all its elements have a valid and appropriate predecessor and successor.

    stabilization
        The process that makes a :term:`cluster` :term:`stable`.
